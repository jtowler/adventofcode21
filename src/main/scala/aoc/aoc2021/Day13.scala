...
class Day13 extends Day(13, 2021) {
  ...
  private val grid = List.tabulate(coords.map(_._2).max + 1, coords.map(_.head).max + 1)((y, x) =>
  ...
  override def answer1(): Any =
    fold(grid, folds.head._1, folds.head._2).map(_.count(_ == true)).sum

  override def answer2(): Any =
    folds.foldLeft(grid)((b, f) => fold(b, f._1, f._2)).map {
      _.map(if _ then '#' else ' ').mkString
    }.mkString("\n")
}